...
# NOTE: agent.planner and agent.error_handler are imported lazily inside methods
...
def _run_generated_code(description: str, speak: Callable | None = None) -> str:
    ...
    try:
        code = generate_text(f"Write Python code to accomplish this task:\n\n{description}", system_instruction=system_instr, force_nvidia=True)
        ...
        print(f"[Executor] CODE Running generated code: {tmp_path}")
    ...
    if tool == "file_controller" and params.get("action") in ("write", "create_file"):
        ...
        if not content or len(content) < 50:
            all_results = [
                ...
                # FIX Bug 6: lowered threshold from 100 → 20 chars
                if v and len(str(v)) > 20 and str(v) not in ("Done.", "Completed.")
            ]
            if all_results:
                combined = "\n\n---\n\n".join(all_results)
...
def _detect_language(text: str) -> str:
    from core.llm_provider import generate_text
    try:
        text_to_detect = f"What language is this text written in? Reply with ONLY the language name in English (e.g. Turkish, English, French).\n\nText: {text[:200]}"
        return generate_text(text_to_detect, model_name="gemini-2.5-flash-lite", force_nvidia=False)
    except Exception:
        return "English"
...
def _translate_to_goal_language(content: str, goal: str) -> str:
    ...
    try:
        ...
        print(f"[Executor] TRANSLATE Translating to: {target_lang}")
        ...
        system_instr = "You are a professional translator."
        translated = generate_text(prompt, system_instruction=system_instr)
...
# FIX Bug 7: added player=None parameter so orchestrator can pass self.ui
def _call_tool(tool: str, parameters: dict, speak: Callable | None,
               player=None) -> str:
...
class AgentExecutor:
    ...
    def execute(
        ...
        # Lazy imports so tests don't trigger LLM/audio initialisation
        from agent.planner import create_plan, replan  # noqa
        from agent.error_handler import analyze_error, generate_fix, ErrorDecision  # noqa
    ...
    def _summarize(self, goal: str, completed_steps: list, speak: Callable | None) -> str:
        ...
        try:
            ...
            summary = generate_text(prompt, model_name="gemini-2.5-flash-lite", force_nvidia=False)
